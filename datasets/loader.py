...
class PairLoader(Dataset):
	def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False):
		...
		self.root_dir = os.path.join(data_dir, sub_dir).replace("\\", "/")
		gt_path = os.path.join(self.root_dir, 'GT').replace("\\", "/")
        
		print(f"Root Directory: {self.root_dir}")
		print(f"GT Path: {gt_path}")
        
		if not os.path.exists(gt_path):
			raise FileNotFoundError(f"The path {gt_path} does not exist. "
                                    "Please verify your dataset structure.")
        
		self.img_names = sorted(os.listdir(gt_path))
		print('This is the list of IMAGEESSS : ' , self.img_names)
		print("-----------------------------------------------------------------------------------------------------\n\n\n")
	...
	def __getitem__(self, idx):
		...
		print('file name passed: ',self.root_dir + '/hazy' + "/" + img_name[:-4] + "_10.png")
		#source_img = read_img(os.path.join(self.root_dir, 'hazy', img_name)) * 2 - 1
		target_img = read_img(self.root_dir + '/GT' + "/" + img_name)  * 2 - 1

		#hazy_img_aug = [] #list which contain
		source_img = read_img(self.root_dir + '/hazy' + "/" + img_name[:-4] + "_10.png") * 2 - 1 #only using the haziest image out of the 1-10 lot
