...
parser.add_argument('--data_dir', default='./datasets/SOTS/', type=str, help='path to dataset') #changed
parser.add_argument('--log_dir', default='./logs/', type=str, help='path to logs')
parser.add_argument('--dataset', default='SOTS/indoor/', type=str, help='dataset name')
...
def train(train_loader, network, DEPTH_NET, criterion_dehaze, criterion_dehaze_cr, criterion_depth,
	...
	for batch in train_loader:
		print("INSIDE BATCH IN TRAIN_LOADER")
		print("\n\n")
		source_img = batch['source'].cpu() #changed .cuda() to .cpu()
		target_img = batch['target'].cpu()
...
if __name__ == '__main__':
	...
	with torch.no_grad():  #previously commented out 
		...
		#encoder_dict = torch.load(encoder_path) #us
		...
		#depth_decoder.load_state_dict(torch.load(decoder_path)) #us
	...
	network = DIACMPN_dehaze_Indoor().cpu()  #changed .cuda() to .cpu()
	DEPTH_NET = DepthNet.DN().cpu()
	deep_estimate_net = UNet().cpu()
